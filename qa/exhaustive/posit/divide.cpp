// divide.cpp: exhaustive functional tests for division
//
...
void GenerateWorstCaseDivision() {
	...
	sw::universal::posit<nbits, es> p_plus_eps(1), p_minus_eps(1), p_result, minpos(sw::universal::SpecificValue::minpos);
	p_plus_eps++;
	p_minus_eps--;
	p_result = p_plus_eps / p_minus_eps;
	if (es < 2) {
		
		std::cout << posit_descriptor.str() << " minpos = " << std::fixed << std::setprecision(nbits) << minpos << std::dec << std::endl;
	}
	else {
		std::cout << posit_descriptor.str() << " minpos = " << std::setprecision(nbits) << minpos << std::endl;
...
try {
	...
	cout << "Posit division validation test generation" << endl;
