...
namespace sw {
	...
	namespace qa {
		...
		template<size_t nbits, size_t es>
		void GenerateUnaryArithmeticTestCase(std::ostream& ostr, int testCase, std::string op, const posit<nbits, es>& rhs, const posit<nbits, es>& pref, const posit<nbits, es>& presult) {
			ostr << std::setw(5) << testCase << " "
				<< " " << op << " "
				<< std::setw(FLOAT_TABLE_WIDTH) << rhs
				<< " == "
		...
		template<size_t nbits, size_t es>
		void GenerateBinaryArithmeticTestCase(std::ostream& ostr, int testCase, std::string op, const posit<nbits, es>& lhs, const posit<nbits, es>& rhs, const posit<nbits, es>& presult) {
			ostr << std::setw(5) << testCase << " "
				<< lhs.get() << " "
				<< rhs.get() << " "
				<< presult.get() << " | "
		...
		template<size_t nbits, size_t es>
		int GenerateValidationTestSet(std::string filename) {
			const int NR_POSITS = (unsigned(1) << nbits);
			int nrOfFailedTests = 0;
			posit<nbits, es> pa, pb, psum, pref;
			std::ofstream ostr;

			filename = filename + "_posit_" + std::to_string(nbits) + "_" + std::to_string(es) + ".vnv";
			std::cerr << "Generating validation data set in " << filename << std::endl;

			ostr.open(filename);
			if (ostr.is_open()) {
				ostr << nbits << " " << es << "    posit<" << nbits << "," << es << ">" << std::endl;
				...
				for (int i = 0; i < NR_POSITS; i++) {
					...
					for (int j = 0; j < NR_POSITS; j++) {
						...
						if (psum != pref) {
							...
							GenerateBinaryArithmeticTestCase(ostr, testCase, "+", pa, pb, psum);
						}
					}
				}
				ostr.close();
				...
				std::cerr << "Unable to open validation output file: " << filename << std::endl;
			}
